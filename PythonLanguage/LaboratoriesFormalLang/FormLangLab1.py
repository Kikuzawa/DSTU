...
def unlimited(rules, dataSymbols):
    for rule in rules:
        # если слева нет нетерминала, то возвращаем false
        if re.search("[" + dataSymbols["nonterminal"] + "]", rule[0]) is None:
            return False
    return True
...
def contextFree(rules, dataSymbols):
    for rule in rules:
        # нет только нетерминала слева
        if re.match("^[" + dataSymbols["nonterminal"] + "]$", rule[0]) is None:
            return False
    return True
    ...
    for rule in rules:
        if re.match("^[" + dataSymbols["nonterminal"] + "]$", rule[0]) is None:
            ...
            for i in range(len(rule) - 1):
                ...
                if re.match("^[" + dataSymbols["nonterminal"] + "]?[" + dataSymbols["terminal"] + "]+$",
                            rule[i + 1]) is None or rightRegular == 2:
                    # не праволинейная и была леволинейной
                    if re.match("^[" + dataSymbols["terminal"] + "]+[" + dataSymbols["nonterminal"] + "]?$",
                                rule[i + 1]) is None or rightRegular == 0:
                        return False
                    # праволинейная
                    else:
                        rightRegular = 2
...
print("Классификация Хомского для грамматики G = (T, N, P, S)\n"
      "Символ 'e' зарезервирован для пустого символа!")

dataSymbols = {"terminal": re.sub(r"(\[\[\]\\\/\^\$\.\|\?\*\+\(\)\{\}])", r"\\\1",
                                  input("Введите терминальные символы T: ").replace(" ", "")) + "e",
               "nonterminal": input("Введите нетерминальные символы N: ").replace(" ", "")}
...
pattern = r"^[" + dataSymbols["alphabet"] + "]+ *-> *" + "(?:[" + dataSymbols["alphabet"] + "]+)(?:(?: *\\| *)(?:[" + \
          dataSymbols["alphabet"] + "]+))*$"
...
if productionRules[0][0] == dataSymbols["start symbol"]:
    ...
    if unlimited(productionRules, dataSymbols):
        ...
        elif contextSensitive(productionRules):
            typeChomsky = 1
    else:
